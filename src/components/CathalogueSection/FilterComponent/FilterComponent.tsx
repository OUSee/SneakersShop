...
export const FilterComponent = () => {
    ...
    const formFilter = (event: any) => {
        ...
        const priceFilteredProducts = productsState.filter((item: Sneaker) => {
            if (
                newFilter.start_price < Number(item.price.replace(" ", "")) &&
                newFilter.end_price > Number(item.price.replace(" ", ""))
            ) {
                return true;
            } else return false;
        });

        const genderFilteredProducts = priceFilteredProducts.filter(
            (item: Sneaker) => {
                if (
                    (newFilter.men && item.gender == "Мужской") ||
                    (newFilter.women && item.gender == "Женский")
                ) {
                    return true;
                } else return false;
            }
        ...
        const sizeFilteredProducts = genderFilteredProducts.filter(
            (item: Sneaker) => {
                if (
                    getSizes(newFilter).some((size) =>
    ...
    return (
        <form onSubmit={formFilter} className={styles.container}>
            ...
            <h3 className={styles.priceTilte}>Цена, руб</h3>
            <div className={styles.pricesContainer}>
                <input
                    id="startPrice"
                    name="start_price"
                    className={styles.controllDigits}
                    type="number"
                    value={prices.min}
                    onChange={(e) => {
            ...
            <h3 className={styles.sexCheckboxTitle}>Пол</h3>
            <fieldset className={styles.checkBoxesContainer}>
                <input
                    ...
                    id="genMen"
                />
                <label htmlFor="genMen">Мужской</label>
                <input
                    name="women"
                    type="checkbox"
                    className="defaultCheckBox"
                    id="genWomen"
                />
                <label htmlFor="genWomen">Женский</label>
            </fieldset>
            <h3 className={styles.SizesCheckboxTitle}>Размер</h3>
            ...
            <button
                type="reset"
                className={styles.setFilterToDefault}
